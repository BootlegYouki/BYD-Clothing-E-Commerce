...
function setupPrimaryImageHandlers() {
    ...
    primaryImageInput.addEventListener('change', async function() {
        ...
        // Mark existing primary image for deletion if present, but don't actually delete it yet
        markExistingPrimaryImageForDeletion();
    ...
    // Helper function to mark existing primary image for deletion
    function markExistingPrimaryImageForDeletion() {
        const existingPrimaryImage = document.querySelector('#primary_image_container');
        if (!existingPrimaryImage || !existingPrimaryImage.dataset.imageId) return;
        
        const imageId = existingPrimaryImage.dataset.imageId;
        
        // Don't call delete-image.php here anymore
        // Just add to tempDeletedImages array if not already there
        if (!tempDeletedImages.includes(imageId)) {
            tempDeletedImages.push(imageId);
            console.log('Primary image marked for deletion:', imageId);
        }
        
        // Update hidden input for deleted images
        document.getElementById('deleted_images').value = tempDeletedImages.join(',');
        
        // Update UI to indicate the image will be replaced
        existingPrimaryImage.classList.add('pending-deletion');
    }
