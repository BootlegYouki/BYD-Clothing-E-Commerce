...
document.addEventListener('DOMContentLoaded', function() {
    // Skip execution if running on localhost
    const hostname = window.location.hostname;
    if (hostname === 'localhost' || hostname === '127.0.0.1') {
        console.log("URL Cleaner skipped: running on localhost");
        return;
    }
    
    // Keep track of the current page to avoid modifying self-references
    ...
    Array.from(links).forEach(link => {
      ...
      // Skip links to the current page to prevent navigation issues
      if (href === currentPage) {
        return;
      }
    ...
    console.log("URL Cleaner executed successfully");
  });